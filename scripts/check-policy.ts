 ...
 * sign+submit via the policy-bound token — the agent's ONLY signing path.
 *
...
type Outcome = string
type Tx = Parameters<OwsXrplSigner['signAndSubmit']>[0]

async function attempt(signer: OwsXrplSigner, label: string, tx: Tx): Promise<Outcome> {
  try {
